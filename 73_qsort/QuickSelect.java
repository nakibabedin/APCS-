...
public class QuickSelect{
  ...
  public static int quickSelect(int[] arr, int loBound, int hiBound, int y){
    ...
    if (newPivotIndex == y - 1){
      ...
      return quickSelect(arr, loBound, newPivotIndex, y);
    }
    else{
      return quickSelect(arr, newPivotIndex, hiBound, y);
    }
  }
  ...
  public static void main(String[] args) {
    ...
    printArr(test1);
    System.out.println("Value of 1st smallest (smallest value): " + quickSelect(test1, 0, 12, 1));

    // highest
    int[] test2 = {6,4,15,8,19,12,11,7,2,0,5,3,20};
    printArr(test2);
    System.out.println("Value of 13th smallest (largest value): " + quickSelect(test2, 0, 12, 13));

    int[] test3 = {6,4,15,8,19,12,11,7,2,0,5,3,20};
    printArr(test3);
    System.out.println("Value of 6th smallest: " + quickSelect(test3, 0, 12, 6));

    int[] test4 = {6,4,15,8,19,12,11,7,2,0,5,3,20};
    printArr(test4);
    System.out.println("Value of 8th smallest: " + quickSelect(test4, 0, 12, 8));

    // BEST case
    // desired value is directly in the middle
    int[] bestCase = {1,0,2};
    printArr(bestCase);
    System.out.println("Value of 1st smallest: " + quickSelect(bestCase, 0, 2, 1));

    // WORST case
    // desired value is at the extremes at each pass
    int[] worstCase = {1,2,0};
    printArr(worstCase);
    System.out.println("Value of 1st smallest: " + quickSelect(worstCase, 0, 2, 1));
